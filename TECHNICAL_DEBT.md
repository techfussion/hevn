...
| **DEBT-08** | **Worker Database Permissions & Secretary Conversational Copy** | **RESOLVED** | Resolved in P2.5.1 — least-privilege `scheduler_service` role RLS policies, `DatabaseCapabilityChecker`, and canonical `ResponseCopyService` | N/A (Feature Complete) | Explicit least-privilege table grants + targeted worker RLS policies, startup capability self-check, deterministic hash-based conversational secretary variation |
