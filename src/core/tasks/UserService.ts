...
export class UserService {
  ...
  async setCheckinHour(userId: string, hour: number): Promise<void> {
    ...
    await getPool().query(`UPDATE users SET preferred_checkin_hour = $1 WHERE id = $2`, [clamped, userId]);
  }
  ...
  async getOrCreate(platform: "telegram" | "whatsapp", platformUserId: string): Promise<User> {
    ...
    const inserted = await pool.query(
      `INSERT INTO users (platform, platform_user_id, timezone)
       VALUES ($1, $2, $3)
       ON CONFLICT (platform, platform_user_id) DO UPDATE SET platform = EXCLUDED.platform
       RETURNING *`,
